...
//     this.pixelData = args.pixelData;
//     this.descenderHeight = args.descenderHeight || 0;
//     this.capHeight = args.capHeight || 0;
//     this.kerning = args.kerning || 0;
// }

// var maxCapHeight = 10;
// var maxDescenderHeight = 0;
// _.chain(letters).keys().each(function(letter) {
//     // var baselineOffset = letters[letter].baselineOffset || 0;
//     // if (letters[letter].pixelData.length + baselineOffset > maxCapHeight) {
//     //     maxCapHeight = letters[letter].pixelData.length + baselineOffset;

//     var descenderHeight = letters[letter].descenderHeight || 0;
//     var capHeight = letters[letter].length - descenderHeight;

//     if (descenderHeight > maxDescenderHeight) {
//         maxDescenderHeight = descenderHeight;
//     }

//     if (capHeight > maxCapHeight) {
...
Paper.prototype.path = function(str) {
    var p = new Path(str);
    this.paths.push(p);
...
Paper.prototype.toString = function() {
    var svg = '<svg width="' + this.width + '" height="' + this.height + '">';
...
var Raphael = require('./mock-raphael');
var letters = require('./letters');


var pix = 50; // Size of one "pixel"
var defsCache = {};

function letter2svg(letter, opts) {
    var data = letters[letter];
    var height = data.blocks.length;
    var width = _.max(_.map(data.blocks, function(str) {
        return str.length;
    }));
    ...
    for (var j = 0; j < height; j++) {
        for (var i = 0; i < width; i++) {
            if (data.blocks[j].charAt(i) !== ' ') {
                var x = i + blockOffsetX;
                var y = j + blockOffsetY;
                // Draw a square
                pathData.push('M');
                pathData.push(variance * normal() + x * pix + pixelOffsetX);
                pathData.push(variance * normal() + y * pix + pixelOffsetY);
                pathData.push('L');
                pathData.push(variance * normal() + x * pix + pix + pixelOffsetX);
                pathData.push(variance * normal() + y * pix + pixelOffsetY);
                pathData.push('L');
                pathData.push(variance * normal() + x * pix + pix + pixelOffsetX);
                pathData.push(variance * normal() + y * pix + pix + pixelOffsetY);
                pathData.push('L');
                pathData.push(variance * normal() + x * pix + pixelOffsetX);
                pathData.push(variance * normal() + y * pix + pix + pixelOffsetY);
                pathData.push('L');
